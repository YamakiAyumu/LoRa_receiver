...
static int gid = 1; // 仮
static const uint16_t lmid = 1; // 仮
const uint16_t groups[] = {1, 2}; // 仮
...
int main() {
    init_controller();
    init_serial(uart_id, UART_TX_PIN, UART_RX_PIN);
    init_lora_module(gid, lmid);

    /*
    スレッド1動かす
    HTTPポストとかBluetooth通信とか外部にデータ投げる
    */ 

    // 受信ループ
    while (true) {
        for (int i = 0; i < sizeof(groups); i++) {
            change_group(groups[i]);
            prepare_comm();
            // delay
            sleep_ms(1000);
            handle_tx_raw(0xFFFF, groups[i], lmid, (const uint8_t *)"", 0); // グループIDのみブロードキャスト送信

            char buffer[128];
            int received_len = handle_rx(buffer, sizeof(buffer), 10000);
            if (received_len < 0) {
                continue;
            }

            handle_tx_raw(0, groups[i], lmid, (const uint8_t *)"\x06", 1); // ACK送信

            if (enqueue_packet(buffer) < 0) {
                continue;
            }
        }
    }
